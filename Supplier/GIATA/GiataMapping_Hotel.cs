    ...
    {       
        ...
        public static XElement GiataResponse(XElement xmlData, XElement availResponse, DataTable HACrncy)
        {
            ...
            try
            {
                ...
                #endregion
                #region added on 260819
                List<string> hotelsAvail = availResponse.Descendants("Table").Select(x => x.Element("HotelID").Value + "_" + x.Element("Suppliers").Value).ToList();
                List<XElement> hotelsWithGiata = xmlData.Descendants("Hotel").Where(x => hotelsAvail.Contains(x.Element("HotelID").Value + "_" + x.Element("SupplierID").Value)).ToList();
                #endregion
                ...
                // added on 260819
                List<XElement> hotelsWithoutGiata = xmlData.Descendants("Hotel").Where(x => !hotelsAvail.Contains(x.Element("HotelID").Value + "_" + x.Element("SupplierID").Value)).ToList();
