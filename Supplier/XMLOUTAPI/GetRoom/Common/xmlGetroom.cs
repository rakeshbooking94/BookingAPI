...
namespace TravillioXMLOutService.Supplier.XMLOUTAPI.GetRoom.Common
{
    public class xmlGetroom : IDisposable
    {
        ...
        public XElement GetRoomAvail_bookingexpressOUT(XElement req, int suppID)
        {
            SupplierId = suppID;
            reqTravayoo = req;
            ...
            {
                #region changed
                List<XElement> htlele = req.Descendants("GiataHotelList").Where(x => x.Attribute("GSupID").Value == Convert.ToString(SupplierId)).ToList();
                for (int i = 0; i < htlele.Count(); i++)
                {
                    ...
                    else
                    {
                        ...
                        else if (SupplierId == 504)
                        {
                            dmc = "NovoDestino";
                        }
                        else if (SupplierId == 533)
                        {
                            dmc = "W2B";
                        }
                        else if (SupplierId == 534)
                        {
                            dmc = "CBHXML";
                        }
                        else if (SupplierId == 627)
                        {
                            dmc = "Adventures";
                        }
                        else if (SupplierId == 628)
                        {
                            dmc = "GTSBeds";
                        }
                        else if (SupplierId == 629)
                        {
                            dmc = "H2GO";
                        }
                    }
        ...
        private XElement getRoom(XElement req,string hotelid)
        {
            ...
            {
                #region Request
                XAttribute agencydet = null;
                try
                {
                    XElement suppliercred = supplier_Cred.getsupplier_credentials(customerid, Convert.ToString(SupplierId));
                    string agencydetail = suppliercred.Attribute("agencyID").Value;
                    agencydet = new XAttribute("agencyDetail", agencydetail + "_" + SupplierId);
                }
                catch { }
                XElement request = new XElement("roomRQ",
                    new XAttribute("customerID", customerid),
                    new XAttribute("agencyID", req.Descendants("SubAgentID").FirstOrDefault().Value), agencydet,
                    ...
                    new XAttribute("rescountryID", req.Descendants("PaxNationality_CountryID").FirstOrDefault().Value),
                    new XAttribute("marketID", req.Descendants("PaxResidenceID").FirstOrDefault().Value == "" ? req.Descendants("PaxNationality_CountryID").FirstOrDefault().Value : req.Descendants("PaxResidenceID").FirstOrDefault().Value),
                ...
                string apiresponse = apireq.xmloutHTTPResponse(request, "RoomAvail", 2, req.Descendants("TransID").FirstOrDefault().Value, customerid, Convert.ToString(SupplierId));
        ...
        private XElement hotelbinding(XElement hotelst)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    List<XElement> dotwrmlst = hotelst.Descendants("room").Where(x => x.Descendants("rate").FirstOrDefault().Attribute("cbID").Value == "5").ToList();
                    ...
                    List<XElement> tbormlst = hotelst.Descendants("room").Where(x => x.Descendants("rate").FirstOrDefault().Attribute("cbID").Value == "21").ToList();
                    ...
                    List<XElement> sunhotelsrmlst = hotelst.Descendants("room").Where(x => x.Descendants("rate").FirstOrDefault().Attribute("cbID").Value == "36").ToList();
                    ...
                    List<XElement> dotwroomlst = null;
                    ...
                    #endregion
                    #region DOTW Rooms
                    if (dotwrmlst.Count() > 0)
                    {
                        dotwroomlst = GetHotelRoomsHProout(dotwrmlst, hotelst.Attribute("code").Value, hotelst.Attribute("currency").Value, dmc, SupplierId);
                    }
                    #endregion
                    ...
                    {
                        mikiroomlst = GetHotelRoomsHProout(mikirmlst, hotelst.Attribute("code").Value, hotelst.Attribute("currency").Value, dmc, SupplierId);
                    }
                    #endregion
                    #region Restel Rooms
                    if (restelrmlst.Count() > 0)
                    {
                        restelroomlst = jacroomgrouping(restelrmlst, hotelst.Attribute("code").Value, hotelst.Attribute("currency").Value).ToList();
                    }
                    #endregion
                    #region W2M Rooms
                    if (w2mrmlst.Count() > 0)
                    ...
                    #endregion
                    #region TBO Holidays Rooms
                    if (tbormlst.Count() > 0)
                    {
                        tboroomlst = GetHotelRoomsHProout(tbormlst, hotelst.Attribute("code").Value, hotelst.Attribute("currency").Value, dmc, SupplierId);
                    }
                    #endregion
                    #region LOH Rooms
                    if (lohrmlst.Count() > 0)
                    ...
                    if (jacrmlst.Count() > 0)
                    {
                        jacroomlst = jacroomgrouping(jacrmlst, hotelst.Attribute("code").Value, hotelst.Attribute("currency").Value).ToList();
                    }
                    #endregion
                    #region Sun Hotels
                    if (sunhotelsrmlst.Count() > 0)
                    {
                        sunhotelsroomlst = GetHotelRoomsHProout(sunhotelsrmlst, hotelst.Attribute("code").Value, hotelst.Attribute("currency").Value, dmc, SupplierId).ToList();
                    }
                    #endregion
                    #region Smy Rooms
                    if (smyrmlst.Count() > 0)
                    {
                        //smyroomlst = jacroomgrouping(smyrmlst, hotelst.Attribute("code").Value, hotelst.Attribute("currency").Value).ToList();
                        smyroomlst = GetHotelRoomsHProout(smyrmlst, hotelst.Attribute("code").Value, hotelst.Attribute("currency").Value, dmc, SupplierId).ToList();
                    }
                    #endregion
        ...
        private IEnumerable<XElement> Getsupplementsout(List<XElement> supplements)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        supplementlst.Add(new XElement("Supplement",
                            new XAttribute("suppId", Convert.ToString(supplements[i].Attribute("suppId").Value)),
                             new XAttribute("suppName", Convert.ToString(supplements[i].Attribute("suppName").Value)),
                             new XAttribute("supptType", Convert.ToString(supplements[i].Attribute("supptType").Value)),
                             new XAttribute("suppIsMandatory", Convert.ToString(supplements[i].Attribute("suppIsMandatory").Value)),
                             new XAttribute("suppChargeType", Convert.ToString(supplements[i].Attribute("suppChargeType").Value)),
                             new XAttribute("suppPrice", Convert.ToString(supplements[i].Attribute("suppPrice").Value)),
                             new XAttribute("suppType", Convert.ToString(supplements[i].Attribute("suppType").Value)))
        ...
        //    <supplements>
        //<supplement suppId="1" suppName="mandatory_tax" supptType="PerStaySupplement" suppIsMandatory="true" suppChargeType="AtProperty" suppPrice="20.20" suppType="PerStaySupplement" sourcekey="JxQdWHgWoJUlrX6zroQmUKEuoBMksVD19ZtZ0eI8E0g="></supplement>
        }
        ...
        private IEnumerable<XElement> jacroomgrouping(List<XElement> rmlst, string hotelcode, string currencycode)
        {
            ...
            for (int i = totalrooms; i >= 1; i--)
            {
                ...
                lst = jacGetitem(i, item, lst, hotelcode, currencycode);
            }
            return lst;
        }
        private List<XElement> jacGetitem(int i, List<XElement> item, List<XElement> lst, string hotelcode, string currencycode)
        {
                    ...
                    foreach (XElement item2 in lst)
                    {
                        ...
                        if (item1.Attribute("MealPlanID").Value == item2.Descendants("Room").FirstOrDefault().Attribute("MealPlanID").Value)
                        {
                            ...
                            new XAttribute("Index", testlst.Count + 1),
                                 new XAttribute("HtlCode", hotelcode), new XAttribute("CrncyCode", currencycode), new XAttribute("DMCType", dmc), new XAttribute("CUID", customerid),
                            item1,
                            romcoll));
                        }
                    }
            ...
            try
            {
                ...
                {
                    var rsult = testlst.GroupBy(x => x.Attribute("TotalRate").Value).Select(y => y.First()).OrderBy(z => z.Attribute("TotalRate").Value).ToList();
                    IEnumerable<XElement> rooms = rsult;
                    testlst = rooms.ToList();
                }
            }
                                                    ...
                                                    new XElement("Offers", ""),
                                                     new XElement("PromotionList",
                                                      //new XElement("Promotions", 
                                                          //room.Descendants("rate").FirstOrDefault().Descendants("offer").FirstOrDefault().Attribute("name").Value != null ? room.Descendants("rate").FirstOrDefault().Descendants("offer").FirstOrDefault().Attribute("name").Value : string.Empty)
                                                          GetHotelpromotionsjacout(room.Descendants("rate").FirstOrDefault().Descendants("offer").ToList())
                                                         ),
                                                     new XElement("CancellationPolicy", ""),
        ...
        private IEnumerable<XElement> GetHotelpromotionsjacout(List<XElement> roompromotions)
        {
            Int32 length = roompromotions.Count();
            List<XElement> promotion = new List<XElement>();
            try
