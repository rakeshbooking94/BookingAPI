...
namespace TravillioXMLOutService.Supplier.XMLOUTAPI.PreBook.Common
{
    public class xmlprebook : IDisposable
    {
        ...
        public XElement prebook_bookingexpressOUT(XElement req,string dmcName, int suppID)
        {
            SupplierId = suppID;
            reqTravayoo = req;
            dmc = dmcName;
            ...
            try
            {
                ...
                string apiresponse = apireq.xmloutHTTPResponse(request, "PreBook", 4, req.Descendants("TransID").FirstOrDefault().Value, customerid, Convert.ToString(SupplierId));
                //string apiresponse = XElement.Load(HttpContext.Current.Server.MapPath(@"~\App_Data\xmlres.xml")).ToString();
        ...
        private XElement hotelbinding(XElement hotelst)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    else if (hotelst.Descendants("room").Where(x => x.Descendants("rate").FirstOrDefault().Attribute("cbID").Value == "5").ToList().Count() > 0)
                    {
                        hotelprebookresponse = prebookrespout(hotelst);
                    }
                    ...
                    else if (hotelst.Descendants("room").Where(x => x.Descendants("rate").FirstOrDefault().Attribute("cbID").Value == "13").ToList().Count() > 0)
                    {
                        hotelprebookresponse = prebookrespout(hotelst);
                    }
                    ...
                    else if (hotelst.Descendants("room").Where(x => x.Descendants("rate").FirstOrDefault().Attribute("cbID").Value == "21").ToList().Count() > 0)
                    {
                        hotelprebookresponse = prebookrespout(hotelst);
                    }
                    ...
                    else if (hotelst.Descendants("room").Where(x => x.Descendants("rate").FirstOrDefault().Attribute("cbID").Value == "36").ToList().Count() > 0)
                    {
                        hotelprebookresponse = prebookrespout(hotelst);
                    }
        ...
        private XElement prebookrespout(XElement supresponse)
        {
            ...
            try
            {
                ...
                XElement contact = null;
                XElement starRating = null;
                if (updateaddress == 1)
                {
                    try
                    {
                        address = new XElement("Address", supresponse.Attribute("address").Value);
                        contact = new XElement("ContactInfo", supresponse.Attribute("ContactInfo").Value);
                        starRating = new XElement("StarRating", supresponse.Attribute("categoryName").Value);
                    }
                    catch { }
                }
                ...
                for (int i = 0; i < supplements.Count(); i++)
                {
                    resplst.Add(new XElement("Supplement",
                         new XAttribute("suppId", Convert.ToString(supplements[i].Attribute("suppId").Value)),
                             ...
                             //new XAttribute("supptType", Convert.ToString("0")),  //Replace 0 to supplements[i].Attribute("SuppId").Value
            ...
            try
            {
                ...
                for (int i = 0; i < cancellationpolicy.Count(); i++)
                {
                    htrm.Add(new XElement("CancellationPolicy", "Cancellation done on after " + cancellationpolicy[i].Attribute("date").Value + "  will apply " + currency + " " + cancellationpolicy[i].Attribute("amount").Value + "  Cancellation fee"
                        ...
                        ,
                        cancellationpolicy[i].Attribute("NoShowPolicy") == null ?
                        new XAttribute("NoShowPolicy", "0") : new XAttribute("NoShowPolicy", cancellationpolicy[i].Attribute("NoShowPolicy").Value)
                        ));
                }
            ...
            try
            {
                foreach (var supplement in supplements)
                {
                    supplementlst.Add(new XElement("Supplement",
                                           new XAttribute("suppId", supplement.Attribute("SuppId").Value),
                                           new XAttribute("suppName", supplement.Attribute("SuppName").Value),
                                           new XAttribute("supptType", supplement.Attribute("SupptType").Value),
                                           new XAttribute("suppIsMandatory", supplement.Attribute("SuppIsMandatory").Value),
                                           new XAttribute("suppChargeType", supplement.Attribute("SuppChargeType").Value),
                                           new XAttribute("suppPrice", supplement.Attribute("SuppPrice").Value),
                                           new XAttribute("suppType", supplement.Attribute("SupptType").Value)));
                    //supplementlst.Add(new XElement("supplement",
