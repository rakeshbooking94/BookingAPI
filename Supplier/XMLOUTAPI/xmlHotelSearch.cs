...
namespace TravillioXMLOutService.Supplier.XMLOUTAPI
{
    public class xmlHotelSearch : IDisposable
    {
        ...
        public List<XElement> hotelSearchXMLOUTAPI(XElement req, string custID, string custName, int suppID, int newTime)
        {
            SupplierId = suppID;
            dmc = custName;
            customerid = custID;
            ...
            try
            {
                ...
                XElement request = new XElement("searchRQ",
                    ...
                    new XAttribute("countryID", req.Descendants("CountryID").FirstOrDefault().Value),
                    new XAttribute("countryCode", req.Descendants("CountryCode").FirstOrDefault().Value),
                ...
                string apiresponse = apireq.xmloutHTTPResponse(request, "Search", 1, req.Descendants("TransID").FirstOrDefault().Value, req.Descendants("CustomerID").FirstOrDefault().Value, Convert.ToString(SupplierId));
                ...
                hotelsearchresponse = bindhotelList(apiresp.Descendants("hotel").ToList()); //arun
                #endregion
            }
            catch(Exception ex)
            {
                ...
                try
                {
                    for (int i = 0; i < length; i++)
                    {
                        try
                        {
                            ...
                            if (htlst[i].Descendants("searchType").Count() > 0)
                            {
                                searchType = htlst[i].Descendants("searchType").FirstOrDefault().Value;
                            }
                            ...
                            htlrslt.Add(new XElement("Hotel",
                                                   ...
                                                   , new XElement("Rooms", "")
                                                   , new XElement("searchType" ,searchType)
                            ));
                            #endregion
                        }
                        catch { }
