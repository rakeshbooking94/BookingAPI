...
namespace TravillioXMLOutService.Supplier.XMLOUTAPI.Cancel
{
    public class xmloutCancel : IDisposable
    {
        ...
        public XElement cancel_bookingexpressOUT(XElement req, int suppID)
        {
            SupplierId = suppID;
            ...
            try
            {
                ...
                try
                {
                    XElement suppliercred = supplier_Cred.getsupplier_credentials(req.Descendants("CustomerID").FirstOrDefault().Value, Convert.ToString(SupplierId));
                ...
                string apiresponse = apireq.xmloutHTTPResponse(request, "Cancel", 6, req.Descendants("TransID").FirstOrDefault().Value, req.Descendants("CustomerID").FirstOrDefault().Value, Convert.ToString(SupplierId));
