...
namespace TravillioXMLOutService.Supplier.XMLOUTAPI.Book.Common
{
    public class xmloutBook : IDisposable
    {
        ...
        public XElement book_bookingexpressOUT(XElement req,int suppID)
        {
            SupplierId = suppID;
            ...
            try
            {
                ...
                request = new XElement("bookingRQ",
                                        ...
                                        new XElement("holder",
                                            new XAttribute("name", firstname.Trim()),
                                            new XAttribute("surname", holderlname.Trim())),
                ...
                string apiresponse = apireq.xmloutHTTPResponse(request, "Book", 5, req.Descendants("TransactionID").FirstOrDefault().Value, customerid, Convert.ToString(SupplierId));
        ...
        private XElement hotelbinding(XElement hotelBooking)
        {
            ...
            try
            {
                ...
                try
                {
                    foreach(XElement rate in hotelBooking.Descendants("rate"))
                    {
                        if (!string.IsNullOrEmpty(rate.Attribute("voucherComments").Value))
                        {
                            special_voucher_remark = special_voucher_remark + ". " + rate.Attribute("voucherComments").Value;
                        }
                    }
                }
                catch { }
            ...
            try
            {
                for (int i = 0; i < rooms.Count(); i++)
                {
                    ...
                    roomlst.Add(new XElement("room",
                        ...
                        new XElement("supplements", supplementbindreq(rooms.Descendants("Supplement").ToList())),
