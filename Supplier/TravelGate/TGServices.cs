...
{
    public class TGServices : IDisposable
    {
        ...
        public List<XElement> HotelSearch(XElement Req, string suplType,string custID)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(HtId))
                {
                    ...
                    else if(supplierID==29)
                    {
                        reqmodel = new SqlModel()
                        {
                            flag = 2,
                            columnList = "HotelID, HotelName, Images, address,latitude,longitude,Star",
                            table = "cosmoHotelDetails",
                            filter = "CityID=" + CityId.ToString() + " AND HotelName LIKE '%" + Req.Descendants("HotelName").FirstOrDefault().Value + "%'",
                            SupplierId = 29
                        };
                    }
                    else if (supplierID == 30)
                    {
                        reqmodel = new SqlModel()
                        {
                            flag = 2,
                            columnList = "cityID+'#'+ HotelID as HotelID, HotelName, Images, address,latitude,longitude,Star",
                            table = "stubaHotelDetails",
                            filter = "CityID=" + CityId.ToString() + " AND HotelName LIKE '%" + Req.Descendants("HotelName").FirstOrDefault().Value + "%'",
                            SupplierId = 30
                        };
                    }
                    else if (supplierID == 32)
                    {
                        reqmodel = new SqlModel()
                        {
                            flag = 2,
                    ...
                    else if (supplierID == 48)
                    {
                        reqmodel = new SqlModel()
                        {
                            flag = 2,
                            columnList = "HotelID, HotelName, Images, address,latitude,longitude,Star",
                            table = "wteHotelDetails",
                            filter = "CityID=" + CityId.ToString() + " AND HotelName LIKE '%" + Req.Descendants("HotelName").FirstOrDefault().Value + "%'",
                            SupplierId = 48
                        };
                    }
        ...
        public List<XElement> SearchHotelList(XElement Req, List<string> HotelListForReq, int tgtimeout,string dmcc, DataTable StaticData)
        {
            ...
            try
            {
                ...
                if (supplierResponse.Descendants("hotelCode").Any())
                {
                    ...
                    //DataTable StaticData = tgd.HotelDetails(hotelCodeString, supplierID);
                    ...
                    try
                    {
                        foreach (var Hotel in HotelGroups)
                        {
                            try
                            {
                                ...
                                if (drow.Count() > 0)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            xmlouttype = "false";
                                            hotelID = hotelID.Split('#')[1].ToString();
                                        }
                                        else if (dmcc == "YALAGO")
                                        {
                                            xmlouttype = "false";
                                        }
                                        else if (dmcc == "WithinEarth")
                                        {
                                            xmlouttype = "false";
                                        }
                                        else
                            ...
                            catch (Exception ex)
                            {
                                #region Exception
                                //CustomException ex1 = new CustomException(ex);
                                //ex1.MethodName = "SearchHotelList1 " + dmcc;
                                //ex1.PageName = "TGServices";
                                //ex1.CustomerID = customerid;
                                //ex1.TranID = Req.Descendants("TransID").FirstOrDefault().Value;
                                //SaveAPILog saveex = new SaveAPILog();
        ...
        public XElement HotelDetail(XElement Req,int supID)
        {
            ...
            try
            {
                ...
                DataTable HotelDetails = tgd.SingleHotelDetails(hotelID, supID);
        ...
        public XElement GetRoomAvail_smyroomOUT(XElement req, XElement SmyRoomsMealPlansfile, int supID)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < htlele.Count(); i++)
                {
                    ...
                    TGServices tgs = new TGServices(supID, customerid);
        ...
        public XElement RoomAvailability(XElement Req, string SupplierType, string htlID, XElement SmyRoomsMealPlansfile, string custIDs)
        {
            ...
            try
            {
                ...
                if (supplierID == 30)
                {
                    hotelids = (SupCityId +"#" + (Req.Descendants("GiataHotelList").Where(x => x.Attribute("GSupID").Value.Equals(supplierID.ToString())).FirstOrDefault().Attribute("GHtlID").Value)).Split(' ').ToArray();
                }
                else
                {
                    hotelids = (Req.Descendants("GiataHotelList").Where(x => x.Attribute("GSupID").Value.Equals(supplierID.ToString())).FirstOrDefault().Attribute("GHtlID").Value).Split(' ').ToArray();
                }
                #region Request Criteria
                var Criteria = new Criteria
                {
        ...
        public XElement CancellationPolicy(XElement Req)
        {
            ...
            try
            {
                ...
                StringBuilder Query = new StringBuilder();
                //deadline added (said by supplier)
                ...
                if (supplierResponse.Descendants("status").FirstOrDefault().Value.ToUpper().Equals("OK"))
                {
                    ...
                    XElement policyTag = supplierResponse.Descendants("cancelPolicy").FirstOrDefault();
                    ...
                    if (refundable)
                    {
                        ...
                        foreach (XElement policy in policyTag.Descendants("cancelPenalties").ToList())
                        {
                            ...
                            if (pAmount > 0)
                            {
                                ...
                                string[] sptdt = policy.Descendants("deadline").FirstOrDefault().Value.Split('T');
                                DateTime lastCancellationDate = DateTime.ParseExact(sptdt[0].ToString().Trim(), "yyyy-MM-dd", CultureInfo.InvariantCulture);
        ...
        public XElement CancellationPolicy_fromdb(XElement Req)
        {
            #region Credentials
            string username = Req.Descendants("UserName").Single().Value;
            string password = Req.Descendants("Password").Single().Value;
        ...
        private object PreBookingRooms(XElement supplierResponse, XElement Req)
        {
            ...
            try
            {
                ...
                #region New CXL Policy
                //////supplierResponse.Descendants("cancelPolicy").FirstOrDefault()
                ////string spresp = supplierResponse.Descendants("response").FirstOrDefault().ToString();
                ////int pFrom = spresp.IndexOf("&lt;ValuationRS") + "&lt;ValuationRS".Length;
                ////int pTo = spresp.LastIndexOf("ValuationRS&gt;");
                ////String finresult = spresp.Substring(pFrom, pTo - pFrom);
                ////string finalpolicynode = "&lt;ValuationRS " + finresult + "ValuationRS&gt;";
                ////string finalcxlnode = HttpUtility.HtmlDecode(finalpolicynode.ToString());
                ////XElement cxlnode = XElement.Parse(finalcxlnode);
                ////XElement policynode = cxlnode.Descendants("CancelPenalties").FirstOrDefault();
                #endregion
        ...
        public XElement PreBookCancellationPolicy_nouse(XElement policyTag, int Nights, double amount, XElement Req)
        {
            XElement Policies = new XElement("CancellationPolicies");
            List<XElement> cxPolicies = new List<XElement>();
            try
            {
                bool refundable = policyTag.Attribute("nonRefundable").Value.ToUpper().Equals("TRUE") ? false : true;
                if (refundable)
                {
                    #region Refundable Polcies
                    foreach (XElement policy in policyTag.Descendants("CancelPenalty").ToList())
                    {
                        double pAmount = PolicyAmount(policy.Element("Penalty").Attribute("type").Value, policy.Element("Penalty").Value, Nights, amount);
                        if (pAmount > 0)
                        {
                            //DateTime lastCancellationDate = DateTime.ParseExact(Req.Descendants("FromDate").FirstOrDefault().Value, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                            //int days = Convert.ToInt32(policy.Element("hoursBefore").Value) / 24;
                            ...
                            //lastCancellationDate = lastCancellationDate.AddDays(-days);
                            //lastCancellationDate = lastCancellationDate.AddHours(-hours);
                            DateTime lastCancellationDate = DateTime.ParseExact(policy.Descendants("Deadline").FirstOrDefault().Value, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                            cxPolicies.Add(new XElement("CancellationPolicy",
                                                    new XAttribute("LastCancellationDate", lastCancellationDate.ToString("dd/MM/yyyy")),
                                                    new XAttribute("ApplicableAmount", pAmount),
                                                    new XAttribute("NoShowPolicy", "0")));
        ...
        public XElement Booking(XElement Req)
        {
            ...
            try
            {
                ...
                var settings = new Settings
               {
                ...
                try
                {
                    ...
                    log.logrequestXML = jsonRequest.ToString();
                    log.logresponseXML = "";
                    SaveAPILog savelog = new SaveAPILog();
                ...
                if (res != null)
                {
                    ...
                    catch(Exception exchk)
                    {
                        #region exception
                        #region Exception
                        CustomException excdk1 = new CustomException(exchk);
                        excdk1.MethodName = "Booking";
                        excdk1.PageName = "TGServices";
                        excdk1.CustomerID = Req.Descendants("CustomerID").FirstOrDefault().Value;
                        excdk1.TranID = Req.Descendants("TransactionID").FirstOrDefault().Value;
                        //APILog.SendCustomExcepToDB(ex1);
                        SaveAPILog saveex = new SaveAPILog();
                        saveex.SendCustomExcepToDB(excdk1);
                        #endregion
                        try
                        {
                            if (SupplierResponse.Descendants("status").FirstOrDefault().Value.Equals("OK"))
                            {
                                double amount = 0;
                                try
                                {
                                    amount = Convert.ToDouble(SupplierResponse.Descendants("net").FirstOrDefault().Value);
                                }
                                catch { }
                                BookingResponse.Add(new XElement("Hotels",
                                                        new XElement("Hotel",
                                                            ...
                                                            new XElement("HotelImgLarge"),
                                                            new XElement("MapLink"),
                                                            new XElement("VoucherRemark", SupplierResponse.Descendants("booking").FirstOrDefault().Element("remarks") == null ? "" : SupplierResponse.Descendants("booking").FirstOrDefault().Element("remarks").Value),
                                                            new XElement("TransID", Req.Descendants("TransactionID").FirstOrDefault().Value),
                                                            new XElement("ConfirmationNumber", SupplierResponse.Descendants("reference").FirstOrDefault().Element("supplier").Value),
                                                            new XElement("Status", "Success"),
                                                            new XElement("PassengerDetail",
                                                            bookingRespRooms(Req.Descendants("PassengersDetail").FirstOrDefault(), amount, Req.Descendants("HotelID").FirstOrDefault().Value)))));
                            }
                        ...
                        catch (Exception exxc2)
                        {
                            #region exception
                            CustomException exxc12 = new CustomException(exxc2);
                            exxc12.MethodName = "Booking";
                            exxc12.PageName = "TGServices";
                            exxc12.CustomerID = Req.Descendants("CustomerID").FirstOrDefault().Value;
                            exxc12.TranID = Req.Descendants("TransactionID").FirstOrDefault().Value;
                            //APILog.SendCustomExcepToDB(ex1);
                            SaveAPILog saveex12 = new SaveAPILog();
                            saveex12.SendCustomExcepToDB(exxc12);
                            BookingResponse.Add(new XElement("ErrorTxt", "Process Error...."));
                            #endregion
                        }
                        #endregion
                ...
                {
                    #region check if booking status was OK
                    if (SupplierResponse.Descendants("status").FirstOrDefault().Value.Equals("OK"))
                    {
                        double amount = 0;
                        try
            ...
            catch (Exception ex)
            {
                ...
                {
                    #region exception
                    CustomException exxc1 = new CustomException(exxc);
                    exxc1.MethodName = "Booking";
                    exxc1.PageName = "TGServices";
                    exxc1.CustomerID = Req.Descendants("CustomerID").FirstOrDefault().Value;
                    exxc1.TranID = Req.Descendants("TransactionID").FirstOrDefault().Value;
                    //APILog.SendCustomExcepToDB(ex1);
                    SaveAPILog saveex1 = new SaveAPILog();
                    saveex1.SendCustomExcepToDB(exxc1);
                    BookingResponse.Add(new XElement("ErrorTxt", "Process Error."));
                    #endregion
                }
            }
        ...
        public double PolicyAmount_nouse(string type, string value, int nights, double bookAmt)
        {
