    ...
    public class EBookingService
    {
        ...
        public List<XElement> HotelAvailability(XElement req, XElement Nationality, string custID, string xtype)
        {
            ...
            try
            {
                ...
                int ebooktimeout = EBookingHelper.timeout();
                ...
                XElement searchRequest = new XElement(soapenv + "Envelope",
                     ...
                     new XElement(soapenv + "Body",
                        new XElement(ns + "HotelGetAvailabilityRQ", new XAttribute("AddHotelDetails", "false"), new XAttribute("AddHotelMinPrice", "true"), new XAttribute("IgnoreHotelOffers", "true"), new XAttribute("Timeout", ebooktimeout),
                        (htlItem != null) ?
                ...
                else
                {
                    ...
                    string resultCode=respHTL.Descendants("AvailHotels").FirstOrDefault().Attribute("ResultCode").Value;
                        ...
                        if (HotelData != null)
                        {
                            if (hotel.Descendants("HotelMinPrice").FirstOrDefault().Attribute("Amount").Value.ModifyToDecimal()>0)
                            {
                                XElement hoteldata = new XElement("Hotel", new XElement("HotelID", HotelData["HotelId"].ToString()),
                                                ...
                                                new XElement("RequestID", resultCode),
        ...
        public XElement HotelDescription(XElement req)
        {
            ...
            try
            {
                ...
                {
                    srvRequest = new EBookingRequest();
                    //EBookingService rs = new EBookingService(req.Descendants("CustomerID").FirstOrDefault().Value);
                    XmlDocument SoapReq = new XmlDocument();
                    XElement htlDetailRequest = new XElement(soapenv + "Envelope",
                     new XAttribute(XNamespace.Xmlns + "soapenv", soapenv),
                     new XAttribute(XNamespace.Xmlns + "ns", "http://tbs.dcsplus.net/ws/1.0/"),
                     new XElement(soapenv + "Header",
                     new XElement(ns + "AuthHeader", new XElement(ns + "ResellerCode", SellerId), new XElement(ns + "Username", UserName), new XElement(ns + "Password", Password), new XElement(ns + "ApplicationId"))),
                     new XElement(soapenv + "Body",
                      new XElement(ns + "GetHotelDetailsRQ",
                          new XElement(ns + "HotelID", req.Descendants("HotelID").FirstOrDefault().Value)
                    )));
                    XElement xmlData = srvRequest.HotelDetailRequest(htlDetailRequest.ToString(), "");
                    if (xmlData.Descendants("HotelDetails").Count() > 0)
                    {

                        string address = xmlData.Descendants("Address").FirstOrDefault().Value;
                        string location = xmlData.Descendants("Location").Count() > 0 ? xmlData.Descendants("Location").FirstOrDefault().Value : string.Empty;
                        string image = xmlData.Descendants("Image").Count() > 0 ? xmlData.Descendants("Image").FirstOrDefault().Attribute("URL").Value : string.Empty;
                        string latitude = xmlData.Descendants("Position").Count() > 0 ? xmlData.Descendants("Position").FirstOrDefault().Attribute("Latitude").Value : string.Empty;
                        string longitude = xmlData.Descendants("Position").Count() > 0 ? xmlData.Descendants("Position").FirstOrDefault().Attribute("Longitude").Value : string.Empty;
                        string desc = (xmlData.Descendants("Descriptions").FirstOrDefault().Descendants("FullDescription").Count() > 0 ? xmlData.Descendants("Descriptions").FirstOrDefault().Descendants("FullDescription").FirstOrDefault().Value : string.Empty) + Environment.NewLine + (xmlData.Descendants("Descriptions").FirstOrDefault().Descendants("Location").Count() > 0 ? xmlData.Descendants("Descriptions").FirstOrDefault().Descendants("Location").FirstOrDefault().Value : string.Empty) + Environment.NewLine + (xmlData.Descendants("Descriptions").FirstOrDefault().Descendants("Facilities").Count() > 0 ? xmlData.Descendants("Descriptions").FirstOrDefault().Descendants("Facilities").FirstOrDefault().Value : string.Empty) + Environment.NewLine + (xmlData.Descendants("Descriptions").FirstOrDefault().Descendants("EssentialInformation").Count() > 0 ? xmlData.Descendants("Descriptions").FirstOrDefault().Descendants("EssentialInformation").FirstOrDefault().Value : string.Empty);
                        XElement gallery = new XElement("Images",
                            from img in xmlData.Descendants("GalleryImage").ToList()
                            select new XElement("Image", new XAttribute("Path", img.Attribute("URL").Value))
                            );
                        XElement facility = new XElement("Facilities",
                            from faci in xmlData.Descendants("Facility").ToList()
                            select new XElement("Facility", faci.Value)
                            );
                        string phone = xmlData.Descendants("Contact").Count() > 0 ? xmlData.Descendants("Contact").FirstOrDefault().Descendants("Phone").Count() > 0 ? xmlData.Descendants("Contact").FirstOrDefault().Descendants("Phone").FirstOrDefault().Value : string.Empty : string.Empty;
                        string email = xmlData.Descendants("Contact").Count() > 0 ? xmlData.Descendants("Contact").FirstOrDefault().Descendants("Email").Count() > 0 ? xmlData.Descendants("Contact").FirstOrDefault().Descendants("Email").FirstOrDefault().Value : string.Empty : string.Empty;
        ...
        public XElement RoomAvailability(XElement roomReq, string xtype, string htlid)
        {
            ...
            try
            {
                string ResultCode = roomReq.Descendants("RequestID").FirstOrDefault().Value;
                string htlCode = htlid;
                srvRequest = new EBookingRequest();
                XElement roomRequest = new XElement(soapenv + "Envelope",
                     new XAttribute(XNamespace.Xmlns + "soapenv", soapenv),
                     new XAttribute(XNamespace.Xmlns + "ns", "http://tbs.dcsplus.net/ws/1.0/"),
                     new XElement(soapenv + "Header",
                     new XElement(ns + "AuthHeader", new XElement(ns + "ResellerCode", SellerId), new XElement(ns + "Username", UserName), new XElement(ns + "Password", Password), new XElement(ns + "ApplicationId"))),
                     new XElement(soapenv + "Body",
                        new XElement(ns + "HotelGetAvailabilityCacheRQ", new XAttribute("AddHotelDetails", "false"), new XAttribute("AddHotelMinPrice", "true"), new XAttribute("IgnoreHotelOffers", "false"),
                            new XElement(ns + "ResultCode", ResultCode),
                            new XElement(ns + "HotelID", htlid)
                     )));
                ...
                XElement respHTL = srvRequest.ServerRequest(roomRequest.ToString(), Url, logmodel);
                if (respHTL.Descendants("Hotel").Count() > 0)
                {
        ...
        public XElement CancellationPolicy(XElement cxlPolicyReq)
        {
            ...
            try
            {
                ...
                XElement respCxlPolicy = srvRequest.ServerRequest(cxlPolicyRequest.ToString(), Url, logmodel);
                decimal RoomGroupRate = respCxlPolicy.Descendants("Package").FirstOrDefault().Descendants("Price").FirstOrDefault().Attribute("Amount").Value.ModifyToDecimal();
                ...
                CxlPolicyResponse.Add(new XElement(soapenv + "Body", cxlPolicyReq, new XElement("HotelDetailwithcancellationResponse",
                        ...
                        GetCxlPolicy(respCxlPolicy.Descendants("Policy").ToList(), RoomGroupRate, cxlPolicyReq.Descendants("FromDate").FirstOrDefault().Value.ConvertToDate())
                        )))))));
            ...
            try
            {
                foreach (var policy in policyList)
                {
                    ...
                    if (policy.Attribute("Type").Value == "standard" || policy.Attribute("Type").Value == "noshow")
                    {
                        ...
                        cxlCharges = policy.Descendants("Charge").FirstOrDefault().Attribute("Amount").Value.ModifyToDecimal();
        ...
        public XElement PreBooking(XElement preBookReq, string xmlout)
        {
            ...
            try
            {
                ...
                int grpIndex = 1;
                decimal RoomGroupRate = respPreBook.Descendants("Package").FirstOrDefault().Descendants("Price").FirstOrDefault().Attribute("Amount").Value.ModifyToDecimal();
                string currency = respPreBook.Descendants("Package").FirstOrDefault().Descendants("Price").FirstOrDefault().Attribute("Currency").Value;
                XElement RoomType = new XElement("RoomTypes", new XAttribute("Index", grpIndex), new XAttribute("TotalRate", RoomGroupRate));
                List<XElement> packagelist = respPreBook.Descendants("Package").FirstOrDefault().Descendants("PackageRoom").ToList();
                int roomSeq = 1;
                foreach (var roompax in preBookReq.Descendants("RoomPax"))
                {
                    var packageroom = packagelist.Find(x => x.Descendants("Occupancy").FirstOrDefault().Attribute("Adults").Value == roompax.Descendants("Adult").FirstOrDefault().Value && x.Descendants("Occupancy").FirstOrDefault().Attribute("Children").Value == roompax.Descendants("Child").FirstOrDefault().Value);
                    int pkIndx = packagelist.FindIndex(x => x.Descendants("Occupancy").FirstOrDefault().Attribute("Adults").Value == roompax.Descendants("Adult").FirstOrDefault().Value && x.Descendants("Occupancy").FirstOrDefault().Attribute("Children").Value == roompax.Descendants("Child").FirstOrDefault().Value);
                    string packageRoomCode = packageroom.Attribute("PackageRoomCode").Value;
                    XElement roomref = packageroom.Descendants("RoomRef").Where(x => x.Attribute("Selected").Value == "true").FirstOrDefault();
                    XElement roomInfo = respPreBook.Descendants("Room").Where(x => x.Attribute("RoomCode").Value == roomref.Attribute("RoomCode").Value).FirstOrDefault();
                    decimal totalRoomRate = roomInfo.Descendants("Price").Count() > 0 ? roomInfo.Descendants("Price").FirstOrDefault().Attribute("Amount").Value.ModifyToDecimal() : RoomGroupRate / totalrooms;
                    string[] others = roomInfo.Descendants("Info").Count() > 0 ? roomInfo.Descendants("Info").FirstOrDefault().Value.Split("Promotion:") : null;
                    ...
                    XElement roomDtl = new XElement("Room", new XAttribute("ID", roomref.Attribute("RoomCode").Value), new XAttribute("SuppliersID", SupplierId), new XAttribute("RoomSeq", roomSeq), new XAttribute("SessionID", PackageCode), new XAttribute("RoomType", roomInfo.Element("Name").Value), new XAttribute("OccupancyID", packageRoomCode),
                                       new XAttribute("OccupancyName", ""), new XAttribute("MealPlanID", ""), new XAttribute("MealPlanName", roomInfo.Element("Board") != null ? roomInfo.Element("Board").Value : "Room Only"), new XAttribute("MealPlanCode", ""), new XAttribute("MealPlanPrice", ""), new XAttribute("PerNightRoomRate", totalRoomRate / nights),
                                       new XAttribute("TotalRoomRate", totalRoomRate), new XAttribute("CancellationDate", ""), new XAttribute("CancellationAmount", ""), new XAttribute("isAvailable", true),
                                       new XElement("RequestID", ResultCode), new XElement("Offers"), new XElement("PromotionList", new XElement("Promotions", Promotion)),
                                       new XElement("CancellationPolicy"), new XElement("Amenities", new XElement("Amenity")),
                                       new XElement("Images", new XElement("Image", new XAttribute("Path", ""))), new XElement("Supplements"),
                                       new XElement(respPreBook.Descendants("PriceBreakdown").FirstOrDefault().Descendants("Room").Count() > 0 ? getPreBookPriceBreakup(respPreBook.Descendants("PriceBreakdown").FirstOrDefault().Descendants("Room").Where(x => x.Attribute("RoomCode").Value == roomref.Attribute("RoomCode").Value).FirstOrDefault().Descendants("DayPrice").ToList()) : getPriceBreakup(nights, totalRoomRate)),
                                       new XElement("AdultNum", packageroom.Descendants("Occupancy").FirstOrDefault().Attribute("Adults").Value),
                                       new XElement("ChildNum", packageroom.Descendants("Occupancy").FirstOrDefault().Attribute("Children").Value));
                    RoomType.Add(roomDtl);
                    roomSeq++;
                ...
                if (respPreBook.Descendants("Remark").Count() > 0)
                {
                    foreach (var rem in respPreBook.Descendants("Remark").ToList())
                    {
                ...
                groupDetails.Descendants("Room").Last().AddAfterSelf(GetCxlPolicy(respPreBook.Descendants("Policy").ToList(), RoomGroupRate, preBookReq.Descendants("FromDate").FirstOrDefault().Value.ConvertToDate()));
                ...
                string checkPayMethod = respPreBook.Descendants("PaymentMethod").Where(x => x.Attribute("Type").Value == "credit").FirstOrDefault().Attribute("Available").Value;

                if (checkPayMethod == "true")
                {
                ...
                {
                    PreBookResponse.Add(new XElement(soapenv + "Body", preBookReqest, new XElement("HotelPreBookingResponse", new XElement("ErrorTxt", "Credit is disabled by administrator."))));
                }
        ...
        public XElement HotelBooking(XElement BookingReq)
        {
            ...
            try
            {
               ...
               XElement BookRequest = new XElement(soapenv + "Envelope",
                ...
                {
                    HotelBookingRes.Add(new XElement(soapenv + "Body", BookReq, new XElement("HotelBookingResponse", new XElement("ErrorTxt", respBook.Descendants("Message").FirstOrDefault().Value))));
                    return HotelBookingRes;
                }
                else
                {
                    string bookStatus = respBook.Descendants("HotelService").FirstOrDefault().Attribute("Status").Value;
                    if (bookStatus == "OK")
                    {
                        decimal amount = 0.0m;
                        try
                        {
                            amount = respBook.Descendants("Price").FirstOrDefault().Attribute("Amount").Value.ModifyToDecimal();
                        }
                        catch
                        {
                        ...
                        XElement BookingRes = new XElement("HotelBookingResponse",
                                               ...
                                               new XElement("ConfirmationNumber", respBook.Descendants("Reservation").FirstOrDefault().Attribute("ReservationID").Value),
        ...
        public XElement BookingCancellation(XElement cancelReq)
        {
            ...
            try
            {
                ...
                int warCnt = resBokCancel.Descendants("Warning").Count();
                if (errCnt > 0 || warCnt > 0)
                {
                ...
                else
                {
                    ...
                    try
                    {
                        amount = resBokCancel.Descendants("Price").FirstOrDefault().Attribute("Amount").Value.ModifyToDecimal();
                    }
                    catch
                    {
                        amount = 0.0m;
                    }
                    BookCXlRes.Add(new XElement(soapenv + "Body", CxlReq, new XElement("HotelCancellationResponse", new XElement("Rooms", new XElement("Room", new XElement("Cancellation", new XElement("Amount", amount), new XElement("Status", resBokCancel.Descendants("HotelService").FirstOrDefault().Attribute("Status").Value == "XX" ? "Success" : "Fail")))))));
                }
