    ...
    public class dr_Roomavail
    {
        ...
        private List<XElement> GetHotelList(List<XElement> htlist, XElement docmealplan, XElement dococcupancy, IEnumerable<XElement> currencycode)
        {
            ...
            {                
                ...
                try
                {
                    for(int i=0;i<1;i++)
                    {
                        List<XElement> roomlist = htlist.Descendants("AccRates").Where(x => x.Descendants("Hotel").FirstOrDefault().Value == hotelid).ToList();
                        ...
                        hotellst.Add(new XElement("Hotel",
                                               ...
                                               new XElement("Currency", Convert.ToString(currencycode.Descendants("CurrencyName").FirstOrDefault().Value)),
                                               new XElement("Offers", "")
                                               , new XElement("Facilities","")
                                               , new XElement("Rooms",
                            GetHotelRooms(htlist, roomlist, docmealplan, dococcupancy, hotelid, currencycode.Descendants("CurrencyName").FirstOrDefault().Value)
                                                   )
                        ));
                    }
                }
        ...
        public List<XElement> GetHotelRooms(List<XElement> htlist, List<XElement> roomlist, XElement docmealplan, XElement dococcupancy, string Hotelcode, string currency)
        {
            ...
            for(int i=0;i<roomlist.Count();i++)
            {
                try
                {
                    List<XElement> docmealplandet = docmealplan.Descendants("d0").Where(x => x.Descendants("MealPlanID").FirstOrDefault().Value == roomlist[i].Descendants("MealPlan").FirstOrDefault().Value).ToList();
                    List<XElement> dococcupancydet = dococcupancy.Descendants("d0").Where(x => x.Descendants("OccupancyID").FirstOrDefault().Value == roomlist[i].Descendants("Occupancy").FirstOrDefault().Value).ToList();
                    List<XElement> room = roomtypes.Descendants("RoomTypes").Where(x => x.Descendants("RoomTypeID").FirstOrDefault().Value == roomlist[i].Descendants("RoomType").FirstOrDefault().Value).ToList();
                    string isavailableval = roomlist[i].Descendants("Availability").FirstOrDefault().Value;
                    ...
                    str.Add(new XElement("RoomTypes", new XAttribute("Index", i + 1),
                        new XAttribute("TotalRate", roomlist[i].Descendants("RatePerStay").FirstOrDefault().Value),
                        ...
                        new XElement("Room",
                             new XAttribute("ID", Convert.ToString(roomlist[i].Descendants("RoomType").FirstOrDefault().Value)),
                             ...
                             new XAttribute("RoomType", Convert.ToString(room.Descendants("RoomTypeName").FirstOrDefault().Value)),
                             new XAttribute("OccupancyID", Convert.ToString(roomlist[i].Descendants("Occupancy").FirstOrDefault().Value)),
                             new XAttribute("OccupancyName", Convert.ToString(dococcupancydet.Descendants("OccupancyName").FirstOrDefault().Value)),
                             new XAttribute("MealPlanID", Convert.ToString(roomlist[i].Descendants("MealPlan").FirstOrDefault().Value)),
                             new XAttribute("MealPlanName", Convert.ToString(docmealplandet.Descendants("MealPlanName").FirstOrDefault().Value)),
                             new XAttribute("MealPlanCode", Convert.ToString(docmealplandet.Descendants("MealPlanCode").FirstOrDefault().Value)),
                             ...
                             new XElement("Promotions", Convert.ToString(roomlist[i].Descendants("Offers").FirstOrDefault().Value))),
