  ...
  new XElement("customer",
     ...
     new XElement("request",
        ...
        public XElement HtlSearchResp(XDocument _dotWReq)
        {
            ...
            XDocument response = _DotwRepo.GetResponse_search(_dotWReq, model);
        ...
        public XElement RmSeaResp(XDocument _dotWReq)
        {
            ...
            XDocument response = _DotwRepo.GetResponse_room(_dotWReq, model);
            bool result = Convert.ToBoolean(response.Descendants("successful").FirstOrDefault().Value);
            if (result)
            {
        ...
        public XElement CxlReq(XElement Req)
        {
            ...
            try
            {
                ...
                model.HotelId = Convert.ToInt64(Req.Descendants("HotelID").FirstOrDefault().Value);
                model.ActionId = 2;
                model.Action = "RoomAvail";
        ...
        public XElement PreBookingSeaResp(XDocument _dotWReq)
        {
            ...
            model.TrackNo = travyoReq.Descendants("TransID").FirstOrDefault().Value;
            model.LogTypeId = 2;
            model.HotelId = Convert.ToInt64(travyoReq.Descendants("HotelID").FirstOrDefault().Value);
      ...
      x.Element("status").Value == "checked").Sum(x => x.Element("total").FirstNode.ToString().StringDecimal());
                ...
                foreach (var item in Restrictlst)
                {
                    ...
                    { 
                        if (item.Attribute("cancel").Value == "true")
                        {
                            try
                            {
                                strText = "Cancellation is restricted for Room" + index + "(" + item.Value + ")" + " from " + item.Attribute("cancelFrom").Value.GetDateTime("yyyy-MM-dd HH:mm:ss").AddDays(-2).ToString("dd/MM/yyyy");
                            }
                            catch
                            {
                                strText = "Cancellation is restricted for Room" + index + "(" + item.Value + ")";
                            }
                            try
                            {
                                strText += " to " + item.Attribute("cancelTo").Value.GetDateTime("yyyy-MM-dd HH:mm:ss").ToString("dd/MM/yyyy") + " .";
                            }
                            catch{}                          
                    ...
                    try
                    {
                        if (item.Attribute("amend").Value == "true")
                        {
                            ...
                            strText = "Amendment is restricted for Room " + index + "(" + item.Value + ")" + " from " + item.Attribute("amendFrom").Value.GetDateTime("yyyy-MM-dd HH:mm:ss").AddDays(-2).ToString("dd/MM/yyyy") + " to " + item.Attribute("amendTo").Value.GetDateTime("yyyy-MM-dd HH:mm:ss").ToString("dd/MM/yyyy") + " .";
                            textItem = new XElement("tariffNotes", strText);
                            textList.Add(textItem);
                    ...
                    {                       
                        strText = "Amendment is restricted for this Room" + index + "(" + item.Value + ")";
                        textItem = new XElement("tariffNotes", strText);
                        textList.Add(textItem);
                    }
        ...
        public void WriteToLogFile1(CustomException logtxt)
        {
        ...
        public bool IsCxlRequired(string TrackNo, string ReqNo)
        {
            ...
            string result = _DotwRepo.GetXml_cancel(model);
