    ...
    public class RTS_HtlSearch
    {
        ...
        public IEnumerable<XElement> HTLResponce(XDocument ele, XElement Req, XElement staticdata, string custID, string dmc)
        {
            ...
            foreach (XElement Srch in Req.Descendants("searchRequest"))
            {
                ...
                //List<XElement> Htlst1=new List<XElement>();
                //foreach(XElement htl in ele.Descendants(ns + "HotelItemInfo"))
                //{
                //    if (Convert.ToInt16(Srch.Element("MinStarRating").Value) <= Convert.ToDecimal(htl.Element(ns + "StarRating").Value == string.Empty ? "0" : htl.Element(ns + "StarRating").Value.Replace("+", string.Empty)) && Convert.ToInt16(Convert.ToDecimal(htl.Element(ns + "StarRating").Value == string.Empty ? "0" : htl.Element(ns + "StarRating").Value.Replace("+", string.Empty))) <= Convert.ToInt16(Srch.Element("MaxStarRating").Value))
                //    {
                //        XElement htldesc = staticdata.Descendants("HtlData").Where(x => x.Element("ItemCode").Value == htl.Element(ns + "ItemCode").Value).FirstOrDefault();

                //        string star = string.Empty;

                //        Htlst1.Add(new XElement("Hotel"
