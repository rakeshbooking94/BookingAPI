        ...
        public XElement HotelAvail_HA(XElement req, string dmc)
        {
            ...
            if (headercheck.Headervalidate(username, password, AgentID, ServiceType, ServiceVersion) == true)
            {
                try
                {
                    ...
                    int expedia = req.Descendants("SupplierID").Where(x => x.Value == "20" && x.Attribute("xmlout").Value == "true").Count();
                    ...
                    int goglobal = req.Descendants("SupplierID").Where(x => x.Value == "27" && x.Attribute("xmlout").Value == "true").Count();
                    ...
                    if (darina > 0 || tourico > 0 || extranet > 0 || hotelbeds > 0 || DOTW > 0 || hotelspro > 0 || travco > 0 || JacTravel > 0 || RTS > 0 || Miki > 0 || restel > 0 || JuniperW2M > 0 || EgyptExpress > 0 || SalTour > 0 || expedia > 0 || tbo > 0 || LOH > 0 || goglobal > 0 || Gadou > 0 || LCI > 0 || SunHotels > 0 || totalstay > 0 || SmyRooms > 0 || AlphaTours > 0 || Hoojoozat > 0 || vot > 0 || ebookingcenter > 0)
                    {
                        ...
                        if (expedia == 1 && Convert.ToInt32(req.Descendants("RoomPax").Count()) < 9)
                        {
                            ...
                            try
                            {
                                custID = req.Descendants("SupplierID").Where(x => x.Value == "20" && x.Attribute("xmlout").Value == "true").FirstOrDefault().Attribute("custID").Value;
                                custName = req.Descendants("SupplierID").Where(x => x.Value == "20" && x.Attribute("xmlout").Value == "true").FirstOrDefault().Attribute("custName").Value;
                            }
                            catch { custName = "HA"; }
                            int customerid = Convert.ToInt32(custID);
                            ExpediaService objRs = new ExpediaService(custID);
                            tid20 = new Thread(new ThreadStart(() => { Expedialst = objRs.HotelAvailability(reqTravillio, custID, custName); }));
                        }
                        ...
                        if (goglobal == 1 && Convert.ToInt32(req.Descendants("RoomPax").Count()) < 10)
                        {
                            ...
                            try
                            {
                                custID = req.Descendants("SupplierID").Where(x => x.Value == "27" && x.Attribute("xmlout").Value == "true").FirstOrDefault().Attribute("custID").Value;
                                custName = req.Descendants("SupplierID").Where(x => x.Value == "27" && x.Attribute("xmlout").Value == "true").FirstOrDefault().Attribute("custName").Value;
                            }
                            catch { custName = "HA"; }
                            GoGlobal rs = new GoGlobal(custID);
                            tid27 = new Thread(new ThreadStart(() => { gogloballst = rs.GoGlobal_HotelAvailabilitySearch(reqTravillio, custName); }));
                        }
