        ...
        public XElement HotelAvail_XMLOUT(XElement req)
        {
            ...
            if (headercheck.Headervalidate(username, password, AgentID, ServiceType, ServiceVersion) == true)
            {
                try
                {
                    ...
                    int expedia = req.Descendants("SupplierID").Where(x => x.Value == "20").Count();
                    int tbo = req.Descendants("SupplierID").Where(x => x.Value == "21").Count();
                    int LOH = req.Descendants("SupplierID").Where(x => x.Value == "23").Count();
                    int goglobal = req.Descendants("SupplierID").Where(x => x.Value == "27").Count();
                    int cosmobeds = req.Descendants("SupplierID").Where(x => x.Value == "29").Count();
                    int stuba = req.Descendants("SupplierID").Where(x => x.Value == "30").Count();
                    int Gadou = req.Descendants("SupplierID").Where(x => x.Value == "31").Count();
                    int yalago = req.Descendants("SupplierID").Where(x => x.Value == "32").Count();
                    ...
                    int welcomebeds = req.Descendants("SupplierID").Where(x => x.Value == "44").Count();
                    ...
                    int wte = req.Descendants("SupplierID").Where(x => x.Value == "48").Count();
                    int iol = req.Descendants("SupplierID").Where(x => x.Value == "50").Count();
                    //int bookingexpress = req.Descendants("SupplierID").Where(x => x.Value == "501").Count();
                    //int etripcenter = req.Descendants("SupplierID").Where(x => x.Value == "502").Count();
                    //int holidaysarabia = req.Descendants("SupplierID").Where(x => x.Value == "503").Count();
                    //int novodestino = req.Descendants("SupplierID").Where(x => x.Value == "504").Count();
                    //int plaza = req.Descendants("SupplierID").Where(x => x.Value == "533").Count();
                    //int cbh = req.Descendants("SupplierID").Where(x => x.Value == "534").Count();
                    //int adventures = req.Descendants("SupplierID").Where(x => x.Value == "627").Count();
                    //int gtsbeds = req.Descendants("SupplierID").Where(x => x.Value == "628").Count();
                    //int h2go = req.Descendants("SupplierID").Where(x => x.Value == "629").Count();
