...
{
    public class JuniperResponses : IDisposable
    {
        ...
        public List<XElement> ThreadedHotelSearch(XElement Req, string xtype)
        {
            ...
            try
            {
                ...
                hotelData = HotelCodes(Req.Descendants("CityID").FirstOrDefault().Value, _JuniperSupplierID.ToString(), HotelId, HotelName, Req.Descendants("MinStarRating").FirstOrDefault().Value, Req.Descendants("MaxStarRating").FirstOrDefault().Value);
                ...
                for (int i = 0; i < Number; i += threadCount)
                {
                    ...
                    if (rangecount == 1)
                    {
                       ...
                       {   
                           new Thread(()=> tr1 = HotelSearch(Req, chunklist.ElementAt(i),citiesWithResult,HotelsForCity, hotelData, timeOut))
                       };
                    ...
                    else if (rangecount == 2)
                    {
                       ...
                       {   
                           new Thread(()=> tr1 = HotelSearch(Req, chunklist.ElementAt(i),citiesWithResult,HotelsForCity, hotelData, timeOut)),
                           new Thread(()=> tr2 = HotelSearch(Req, chunklist.ElementAt(i+1),citiesWithResult,HotelsForCity, hotelData, timeOut))
                       };
                        threadedlist.ForEach(t => t.Start());
                        threadedlist.ForEach(t => t.Join(timeOut));
                        threadedlist.ForEach(t => t.Abort());
                    ...
                    else if (rangecount == 3)
                    {
                       ...
                       {   
                           new Thread(()=> tr1 = HotelSearch(Req, chunklist.ElementAt(i),citiesWithResult,HotelsForCity, hotelData, timeOut)),
                           new Thread(()=> tr2 = HotelSearch(Req, chunklist.ElementAt(i+1),citiesWithResult,HotelsForCity, hotelData, timeOut)),
                           new Thread(()=> tr3 = HotelSearch(Req, chunklist.ElementAt(i+2),citiesWithResult,HotelsForCity, hotelData, timeOut))
                       };
                        threadedlist.ForEach(t => t.Start());
                        threadedlist.ForEach(t => t.Join(timeOut));
                        threadedlist.ForEach(t => t.Abort());
                    ...
                    else if (rangecount == 4)
                    {
                        threadedlist = new List<Thread>
                       {
                           ...
                           new Thread(()=> tr2 = HotelSearch(Req, chunklist.ElementAt(i+1),citiesWithResult,HotelsForCity, hotelData, timeOut)),
                           new Thread(()=> tr3 = HotelSearch(Req, chunklist.ElementAt(i+2),citiesWithResult,HotelsForCity, hotelData, timeOut)),
                           new Thread(()=> tr4 = HotelSearch(Req, chunklist.ElementAt(i+3),citiesWithResult,HotelsForCity, hotelData, timeOut))
                    ...
                    else if (rangecount == 5)
                    {
                        threadedlist = new List<Thread>
                       {
                           ...
                           new Thread(()=> tr3 = HotelSearch(Req, chunklist.ElementAt(i+2),citiesWithResult,HotelsForCity, hotelData, timeOut)),
                           new Thread(()=> tr4 = HotelSearch(Req, chunklist.ElementAt(i+3),citiesWithResult,HotelsForCity, hotelData, timeOut)),
                           new Thread(()=> tr5 = HotelSearch(Req, chunklist.ElementAt(i+4),citiesWithResult,HotelsForCity, hotelData, timeOut))
                       };
                        threadedlist.ForEach(t => t.Start());
                        threadedlist.ForEach(t => t.Join(timeOut));
                        threadedlist.ForEach(t => t.Abort());
        ...
        public List<XElement> HotelSearch(XElement Req, List<string> chunkthread, DataTable CitiesWithHotels, List<XElement> HBECodes, XElement hotelData, int timeOut)
        {
                                                ...
                                                new XAttribute("End", JuniperDate(Req.Descendants("ToDate").First().Value))),
                                                 ...
                                                 BindHotelCodes(chunkthread)),
                                                      ...
                                                      new XElement(soap + "AdvancedOptions",
                                                          ...
                                                          new XElement(soap + "ShowOnlyBestPriceCombination", "true"),
                                                          new XElement(soap + "ShowHotelInfo", "true"),
                                                          new XElement(soap + "ShowBreakdownPrice", "false"),
                                                           new XElement(soap + "ShowOnlyAvailable", "true"),
                                                            new XElement(soap + "ShowCancellationPolicies", "false"),
                                                            new XElement(soap + "TimeOut", timeOut >= 15000 ? 15000 : timeOut)))))));
                #endregion
        ...
        private XElement HotelSearchResponse(XDocument toRequest, XElement travReq)
        {
            DateTime starttime = DateTime.Now;
            XElement response = null;
            try
            {
                XElement serverResponses = serverRequest.JuniperResponseSearch(toRequest, "HotelAvail", _JNPRHotSearURL, Convert.ToInt64(customerid), Convert.ToString(travReq.Descendants("TransID").FirstOrDefault().Value), _JuniperSupplierID);
                #region Log Save
                XElement responseRNS = removeAllNamespaces(serverResponses);
                ...
                catch (Exception exApi)
                {
                    ...
                    ex1.MethodName = "HotelSearchResponseSave";
                    ex1.PageName = "JuniperResponses";
                ...
                if (responseRNS.Descendants("HotelResult").Count() > 0)
                {
                    try
                    {
                        //alpha issue 14-10-2019 string cityID = responseRNS.Descendants("HotelResult").FirstOrDefault().Attribute("DestinationZone").Value;
                        //DataTable StaticData_All = new DataTable();
                        ...
                        int minstar = Convert.ToInt16(travReq.Descendants("MinStarRating").First().Value);
                        int maxstar = Convert.ToInt16(travReq.Descendants("MaxStarRating").First().Value);

                        foreach (XElement ResHotels in responseRNS.Descendants("HotelResult"))
                        {
                            try
                            {
                                int supStar = 0;
                                supStar = StarRating(ResHotels.Element("HotelInfo").Element("HotelCategory"));
                                if (supStar >= minstar && supStar <= maxstar)
                                {
                                    string hotelcode = Convert.ToString(ResHotels.Attribute("Code").Value);
                                    DataRow[] row = StaticData_All.Select("HotelID = " + "'" + hotelcode + "'");
                                    if (row.Count() > 0)
                                    {
                                        Hotels.Add(new XElement("Hotel",
                                                                ...
                                                                new XElement("HotelImgSmall", Convert.ToString(row[0]["ImageURL"].ToString())),
                                                                new XElement("HotelImgLarge", Convert.ToString(row[0]["ImageURL"].ToString())),
        ...
        private XElement RoomAvailability(XElement Req, string xtype, string htlcode, string reqid, string customerid)
        {
            try
            {
                XDocument HotelsearchRequest = null;
                ...
                {
                    XElement resp = null;
                    #region Check availability from Supplier
                    try
                    {
                        #region Request XML
                        HotelsearchRequest = new XDocument(
                                          new XDeclaration("1.0", "utf-8", "yes"),
                                          new XElement(soapenv + "Envelope",
                                              new XAttribute(XNamespace.Xmlns + "soapenv", soapenv),
                                              new XAttribute(XNamespace.None + "xmlns", soap),
                                                   ...
                                                   new XElement(soap + "SearchSegmentHotels",
                                                        ...
                                                        new XAttribute("End", JuniperDate(Req.Descendants("ToDate").First().Value))),
                                                         ...
                                                         new XElement(soap + "HotelCodes",
                                                             new XElement(soap + "HotelCode", hid))),
                                                              new XElement(soap + "RelPaxesDist", bindRomPax(Req.Descendants("RoomPax").ToList()))),
                                                              new XElement(soap + "AdvancedOptions",
                                                                  new XElement(soap + "UseCurrency", "USD"),                                                                 
                                                                  new XElement(soap + "ShowHotelInfo", "true"),
                        ...
                        XElement serverResponses = serverRequest.JuniperResponseSearch(HotelsearchRequest, "HotelAvail", _JNPRHotSearURL, Convert.ToInt64(customerid), Convert.ToString(Req.Descendants("TransID").FirstOrDefault().Value), _JuniperSupplierID);
                        XElement responseRNS = removeAllNamespaces(serverResponses);
                        resp = responseRNS.Descendants("HotelResult").FirstOrDefault();                        
                    }
                    catch { }
                    #endregion
                    ...
                    try
                    {
                        ...
                        log.SupplierID = _JuniperSupplierID;
                        log.logrequestXML = HotelsearchRequest.ToString();
                        log.logresponseXML = resp == null ? "" : resp.ToString();
                        log.LogTypeID = 2;
        ...
        public XElement hoteldetails(XElement req,int supid)
        {
            XElement response = null;
            #region Credentials
            string username = req.Descendants("UserName").Single().Value;
            ...
            try
            {
                ...
                #endregion
                response = HotelDetailsResponse(restelreq, req,supid);
            }
            catch (Exception ex)
            {
                #region Exception
        ...
        #region Response
        public XElement HotelDetailsResponse(XDocument supplierReq, XElement travayooReq,int supid)
        {
            XElement response = null;
            ...
            string[] splitid = hid.Split(new char[] { '_' });
            string hotelID = string.Empty, Supl_ID = supid.ToString();
        ...
        public XElement cancellationPolicy(XElement Req)
        {
            ...
            try
            {
                ...
                XElement supplierResponse = LogXMLs(Req.Descendants("TransID").First().Value, 2, _JuniperSupplierID).Descendants("Response")
                ...
                XElement tryRoom=null;
                XElement response = null;
                XElement cxlRoom = null;
                if (!HotelOption.Descendants("CancellationPolicy").Any())
                {
                    #region Request XML
                    ...
                    #endregion
                    response=PreBookingCxlPolicyResponse(Req, BookingRulesRequest);
                    if (response.Descendants("HotelOption").Any() && response.Descendants("HotelOption").First().Attribute("Status").Value.Equals("OK"))
                    {
                          tryRoom = PreBookRooms(response.Descendants("HotelOptions").First(), Req);

                          cxlRoom = newCxlPolicy(response.Descendants("HotelOption").FirstOrDefault().Descendants("CancellationPolicy").FirstOrDefault(), tryRoom.Descendants("Price").ToList(), Req.Descendants("FromDate").FirstOrDefault().Value);
                ...
                else
                {
                    ...
                    tryRoom = PreBookRooms(itemmm, Req);
                    cxlRoom = newCxlPolicy(HotelOption, tryRoom.Descendants("Price").ToList(), Req.Descendants("FromDate").FirstOrDefault().Value); 
                }               
        ...
        public XElement PreBookRooms(XElement JunPRooms, XElement travReq)
        {
            ...
            List<XElement> room = JunPRooms.Descendants("HotelOption").ToList();
            ...
            for (int k = 0; k < room.Count(); k++)
            {
                ...
                bool roomflag = IsRoomRate(room[k]);
                if (pricezero == true)
                {
                    roomflag = false;
                }
                ...
                foreach (XElement roompax in travReq.Descendants("RoomPax"))
                {
                    ...
                    string[] strSource = room[k].Descendants("HotelRoom").First().Attribute("Source").Value.Split(new char[] { ',' });
                    ...
                    foreach (XElement hotelRoom in room[k].Descendants("HotelRoom"))
                    {
                        //string rmlist = null; 
                        ...
                        for (int i = 0; i < strSrc.Length; i++)
                        {
                            if (roompax.Element("id").Value.Equals(strSrc[i]))
                            {
                                ...
                                decimal TotalAmounts = Convert.ToDecimal(room[k].Descendants("TotalFixAmounts").FirstOrDefault().Attribute("Nett").Value);

                                XElement pricedetail = PriceBreakup(room[k].Descendants("Prices").First(), strSrc[i], nights, Convert.ToDateTime(JuniperDate((travReq.Descendants("FromDate").First().Value))), travReq.Descendants("RoomPax").Count(), roomflag, TotalAmounts);
                                ...
                                if (room[k].Descendants("Board").Count() > 0)
                                {
                                    var item = room[k].Descendants("Board").FirstOrDefault();
                                    mealType = item.Attribute("Type") != null ? item.Attribute("Type").Value : "";
                                    mealName = item.Value;
                                }
                                roomlist.Add(new XElement("Room",
                                                                new XAttribute("ID", room[k].Element("BookingCode").GetValueOrDefault("") + "_" + hotelRoom.Descendants("Name").FirstOrDefault().Value),
                                                                  ...
                                                                  new XElement("RequestID", room[k].Element("BookingCode").GetValueOrDefault("")),
                                                                  new XElement("Offers"),
                                                                  RoomPromotion(room[k].Descendants("HotelOffer")),
                                                                  new XElement("CancellationPolicy"),
                                                                  new XElement("Amenities",
                                                                      new XElement("Amenity")),
                                                                  new XElement("Images", new XElement("Image", new XAttribute("Path", ""))),
                                                                  SupplementTag(room[k].Descendants("HotelSupplement")),
                                                                   pricedetail,
                ...
                #region Check 0 Price
                int checkprc = roomlist.Where(x => x.Attribute("TotalRoomRate").Value == "0").Count();
                if (checkprc > 0)
                {
                    k--;
                    pricezero = true;
        ...
        public XElement bookingResponse(XDocument RestelRequest, XElement travReq)
        {
            ...
            try
            {
                try
                {
                    ...
                    logreq.customerID = Convert.ToInt64(travReq.Descendants("CustomerID").FirstOrDefault().Value);
                    logreq.TrackNumber = travReq.Descendants("TransactionID").FirstOrDefault().Value;
                    logreq.SupplierID = _JuniperSupplierID;
                    logreq.logrequestXML = RestelRequest.ToString();
                    logreq.logresponseXML = "";
                    logreq.LogType = "Book";
                    logreq.LogTypeID = 5;
                    logreq.StartTime = DateTime.Now; 
                    logreq.EndTime = DateTime.Now;
                        ...
                        foreach (XElement hotelRoom in room[k].Descendants("HotelRoom"))
                        {
                            ...
                            for (int i = 0; i < strSrc.Length; i++)
                            {
                                if (roompax.Element("id").Value.Equals(strSrc[i]))
                                {
                                    ...
                                    decimal TotalAmounts = Convert.ToDecimal(room[k].Descendants("TotalFixAmounts").FirstOrDefault().Attribute("Nett").Value);
                                    XElement pricedetail = PriceBreakup(room[k].Descendants("Prices").First(), strSrc[i], Convert.ToInt16(travReq.Descendants("Nights").First().Value), Convert.ToDateTime(JuniperDate((travReq.Descendants("FromDate").First().Value))), travReq.Descendants("RoomPax").Count(), roomflag, TotalAmounts);
                                    ...
                                    roomlist.Add(new XElement("Room",
                                        new XAttribute("ID", room[k].Attribute("RatePlanCode").Value + "_" + hotelRoom.Descendants("Name").FirstOrDefault().Value),
                                        new XAttribute("SuppliersID", _JuniperSupplierID.ToString()),
                                        new XAttribute("RoomSeq", RoomSqlCounter++),
                                        new XAttribute("SessionID", reqid),
                                        new XAttribute("RoomType", hotelRoom.Descendants("Name").First().Value),
        ...
        public XElement HotelCodes(string CityCode, string SupplierID, string HotelCode, string HotelName,string minRating, string maxRating)
        {
            ...
            //}
            XElement DataList = new XElement("Hotels");
            DataTable hotelsList = jdata.GetHotelsList(CityCode, SupplierID, HotelCode, HotelName,minRating,maxRating);
        ...
        {
            DataTable hotelsList = jdata.GetHotelsList(CityCode, SupplierID, HotelCode, HotelName,"0","5");
            List<XElement> Hotels = new List<XElement>();
            //List<string> NewResponse = new List<string>();
            for (int i = 0; i < hotelsList.Rows.Count; i++)
            {
            ...
            if (!Promotionlst.IsNullOrEmpty())
            {
                try
                {
                    var result = from promo in Promotionlst
                                 select new XElement("Promotions", promo.Element("Name") == null ? promo.Element("Description").Value : promo.Element("Name").Value);
                    promoItem = new XElement("PromotionList", result);
                }
                catch { promoItem = new XElement("PromotionList", new XElement("Promotions", null)); }
            }
            else
            {
        ...
        #region PreBookGetCxlPolicy
        //Date : 29/01/2021
        //Name : Rahul Kumar
        //Desc :  when Cancelation Policy Null in Room  then call PreBookingCxlPolicyResponse to get Cancelation Policy
        public XElement PreBookingCxlPolicyResponse(XElement travReq, XDocument prebookreq)
        {
            XElement resp = null;
            XElement response = new XElement("HotelPreBookingResponse");
            DateTime starttime = DateTime.Now;
