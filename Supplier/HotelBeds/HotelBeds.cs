                ...
                using (var client = new WebClient())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        try
                        {
                            ...
                            saveapi.SaveAPILogs_search(log);
                        }
                        catch { }
                        CustomException ex1 = new CustomException(ex);
                                ...
                                if (row.Count() > 0)
                                {
                                    ...
                                    {                                        
                                        try
                                        {
                                            ...
                                            {
                                                XElement imgpth = hotelstatic.Descendants("image").Where(x => x.Attribute("imageTypeCode").Value == "GEN").OrderBy(x=>Convert.ToInt16(x.Attribute("order").Value)).FirstOrDefault();
                                            ...
                                            {
                                                XElement imgpth = hotelstatic.Descendants("image").Where(x => x.Descendants("type").Attributes("code").FirstOrDefault().Value == "GEN").OrderBy(x => Convert.ToInt16(x.Attribute("order").Value)).FirstOrDefault();
                                                //string pth = imgpth.Attribute("path").Value;
                                                if (imgpth != null)
                                                {
                                                    smallimagepath = "http://photos.hotelbeds.com/giata/medium/" + imgpth.Attribute("path").Value;
