...
namespace TravillioXMLOutService.Supplier.SunHotels
{
    public class SunHotelsResponse
    {
        ...
        #region Response
        public XElement HotelDetailsResponse(XDocument restelreq, XElement travayooReq,int supid)
        {
            DataTable cityMapping = jdata.CityMapping(travayooReq.Descendants("CityID").FirstOrDefault().Value, supid.ToString());
            string suppliercity = cityMapping.Rows[0]["SupCityId"].ToString();
