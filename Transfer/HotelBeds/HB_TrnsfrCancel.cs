...
namespace TravillioXMLOutService.Transfer.HotelBeds
{
    public class HB_TrnsfrCancel
    {
        ...
        public async Task<XElement> CancelTransfer(XElement req)
        {
            ...
            if (headercheck.Headervalidate(username, password, AgentID, ServiceType, ServiceVersion) == true)
            {
                try
                {
                    int hotelbeds = req.Descendants("supplier").Where(x => x.Attribute("supplierid").Value == "10").Count();
                    if (hotelbeds == 1)
                    {


                        string customerId = req.Descendants("cancellationRequest").Attributes("customerId").FirstOrDefault().Value;
                        HotelBedService hbreq = new HotelBedService(customerId);
                        XElement SearReq = req.Descendants("cancellationRequest").FirstOrDefault();
                        XElement response =await hbreq.CancelBookingAsync(SearReq);
                    ...
                    else
                    {
                        ...
                        IEnumerable<XElement> request = req.Descendants("cancellationRequest");
                        ...
                        XElement searchdoc = new XElement(
                          new XElement(soapenv + "Envelope",
                               ...
                               new XElement("cancellationResponse",
